...
To clarify, we look at router $n-1$'s and $n-2$'s port 0 (west) queue sizes 
for router $n$ as well as router $n+1$'s and $n+2$'s port 2 (east) queue sizes.
Of course, this arithmetic is done under modulo 8. 
Given this congestion factor, we determine whether we want to route packets 
through the path with the longer hop count. \par
...
In the other cases so far, we simply add the congestion as a one router hop 
cost.
So if a channel going east has a 3 queue length delay at the first router 
